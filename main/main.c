 ...
 * @file main.c
 * @author Nguyen Hoang
 * @brief contain two main tasks of adc_measure_task and thingspeak_task
 *        Also include function for user_timer.h header file and function of ISR for Timer
 * @version 0.1
 * @date 2021-06-28
 * 
 * @note The wifi AP information is stored in user_wifi.h, please change CONFIG_WIFI_SSID and CONFIG_WIFI_PASS
 * @note Thingspeak API Key is stored in thingspeak.h, please change THINGSPEAK_API_KEY
 * @note Thingspeak request is stored in thingspeak.c, please change start_request and end_request
 * @note Period of ADC function and timer divider is stored in user_timer.h, please change ADC_PERIOD and TIMER_DIVIDER
 * 
...
static SemaphoreHandle_t xSemaphoreADC;     //binary => max count = 1
static uint16_t xSemaphoreADC_value[4];
/* Event variable which is used for timer to control the frequency of adc */
/* Danh dau timer da chay xong */
static EventGroupHandle_t xEventGroupADC;

/*********** Timer Function *********************/

/**
 * @brief Timer group0 Interrupt handler
 *
 * @note:
...
//     /* Finish init SD card */
    ...
    while(1) {
        ...
        //waiting until timer has expired => wait until TIMER_FINISH_BIT is set
        bits = xEventGroupWaitBits(xEventGroupADC, TIMER_FINISH_BIT, pdFALSE, pdFALSE, portMAX_DELAY);
        /* xEventGroupWaitBits() returns the bits before the call returned, hence we can test which event actually
         * happened. */
        if(bits & TIMER_FINISH_BIT) {   //if TIMER_FINISH_BIT is set
        ...
        if(xSemaphoreADC != NULL) {
            if(uxSemaphoreGetCount(xSemaphoreADC) == 0) {
                //Semaphore is not available to give => take semaphore to increase count to give new record
                xSemaphoreTake(xSemaphoreADC, 0);       //2nd variable: delay time = 0
            }
...
void app_main(void)
{
    ESP_ERROR_CHECK(nvs_flash_init());                  //init esp memory flash - code exe on external flash

    ESP_ERROR_CHECK(esp_netif_init());                  //init internet interface
    ESP_ERROR_CHECK(esp_event_loop_create_default());   //create event loop for wifi connection
    //ESP has Event Loop Library
    //Example: WIFI_DISCONNECTED_EVENT => function: try reconnecting to wifi
    //Example: WIFI_GOT_IP: => function: display info about wifi

    ESP_ERROR_CHECK(wifi_connect());                    //start connecting wifi
    ...
    //function address, function name, stack depth, param, priorities, task handle, core
}
